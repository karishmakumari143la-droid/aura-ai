  ...
  const integrationsList = [
{
    ...
    try {
      ...
      if (!sessionUser) {
        return res.status(401).json({
          ...
          message: 'AURA AI session is required.'
        });
      }
  ...
  // Secure background task creation proxy.
  // The authenticated AURA session determines task ownership.
  app.post('/api/brain/background-task', async (req: Request, res: Response) => {
    try {
      const sessionUser = (req as any).user || getSessionUser(req);
      ...
      const { title, command, scope_id, grant_id } = req.body || {};
      ...
      console.error('[AURA Background Task] Creation failed:', err);
      return res.status(500).json({
        success: false,
        error: 'BACKGROUND_TASK_CREATE_FAILED',
        message: err?.message || 'Failed to create background task.'
      });
    }
  });

  // Secure background task status proxy.
  // The authenticated AURA session determines task ownership.
  app.get('/api/brain/background-task/:taskId', async (req: Request, res: Response) => {
    try {
      const sessionUser = (req as any).user || getSessionUser(req);
      ...
      console.error('[AURA Background Task] Status lookup failed:', err);
      return res.status(500).json({
        success: false,
        error: 'BACKGROUND_TASK_STATUS_FAILED',
        message: err?.message || 'Failed to retrieve background task status.'
      });
    }
  });
