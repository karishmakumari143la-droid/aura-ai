  ...
  return (
    <header className="border-b border-white/[0.08] bg-[#02050B]/90 backdrop-blur-2xl sticky top-0 z-40 px-4 sm:px-6 h-14 flex items-center justify-between transition-all select-none">
      ...
      <div className="flex items-center gap-2 sm:gap-2.5">
        ...
        {/* Voice status — configured in AURA Voice Studio */}
        <button
          type="button"
          onClick={() => onSelectTab('settings')}
          className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold transition border bg-slate-950/80 border-white/[0.08] text-slate-300 hover:text-white hover:border-cyan-500/40"
          title="Open AURA Voice Studio"
        >
          <span className={`relative flex h-2 w-2 ${isVoiceActive ? '' : 'opacity-60'}`}>
            <span className={`relative inline-flex rounded-full h-2 w-2 ${
              isVoiceActive ? 'bg-teal-400' : 'bg-slate-500'
            }`} />
          </span>
          <Mic className={`w-3.5 h-3.5 ${
            isVoiceActive ? 'text-teal-300' : 'text-slate-500'
          }`} />
          <span className="hidden sm:inline text-[11px]">
            {isVoiceActive ? 'Voice Ready' : 'Voice Setup'}
          </span>
        </button>
